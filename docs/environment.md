...
Your own [Umami](https://umami.is), on the public pages, reporting to a server
you run. **Not telemetry, and not routed through it.** The distinction is who
is asking: telemetry is Balancia asking your instance for a weekly summary,
which is why its address is compiled in and an administrator has to consent to
it. This is your landing page reporting to your own analytics — the same
category of decision as pointing `SMTP_HOST` at a mail relay, and the same
reason the address is yours to set.

Off unless both variables are set. Setting one without the other fails at
startup rather than collecting nothing quietly.

### `UMAMI_SCRIPT_URL`

Absolute URL of the tracker script, usually `/script.js` on your Umami.

**Must be HTTPS outside localhost** — the page's Content-Security-Policy sends
`upgrade-insecure-requests`, so an `http://` script is fetched over `https://`
and simply fails if that is not served. Startup says so rather than leaving you
with an empty dashboard.

The **origin** of this URL is also the only third-party host the CSP admits, in
`connect-src`. It is derived rather than configured separately, so the address
the tracker posts to and the address the policy permits cannot drift apart.

### `UMAMI_WEBSITE_ID`

The UUID Umami shows for the website. Checked for that shape at startup:
Umami accepts a request carrying a wrong ID and files it under nothing, which
is a failure you would otherwise find by waiting a week for data.
...
### Where the tracker runs, and where it does not

On `/`, `/sign-in`, `/register` and `/register/done`. Nowhere else.

That is a property of **which pages the script is on**, not of how Umami is
configured. Every other page in Balancia names a group or an expense in its
path — `/groups/{groupId}/expenses/{expenseId}` — and a page view carries the
path. No tracker setting makes that safe to hand to a third party, so those
pages have no tracker. A test fails the build if the component is imported
anywhere but those two files.

The tag is mounted with `data-exclude-search`, because two of those public
pages carry a group ID in the query string (`/sign-in?next=/groups/{id}` when a
signed-out reader opens a group link, `/register/done?group={id}` after
registration). It also sets `data-do-not-track`, which honours the browser's
signal.

**Your privacy policy, not Balancia's.** Turning this on means visitors to your
instance are counted by a third party you chose. Umami sets no cookie and
stores no IP address, but it does hash the IP together with the user agent and
a rotating salt to recognise a visit for a day — which is a pseudonym, not
anonymity. Say so wherever you say such things.
